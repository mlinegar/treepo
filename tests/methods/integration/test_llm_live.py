"""Live-server integration test for `treepo.methods` against a real vLLM endpoint.
...
Enable with:
    ...
    TT_RUN_LIVE_TESTS=1 \\
      python -m pytest \\
      tests/methods/integration/test_llm_live.py -v

Two checks:

1. **Direct DSPy roundtrip.** Build a ``DSPyFamily`` via the
   ``treepo.methods`` registry with ``lm_transport="litellm"`` pointed at
...
What this proves: the path from ``treepo.methods.run(...)`` → registered
