...
from treepo.methods import canonical_defaults as cd

CONFIGS = TREEPO_ROOT / "configs" / "research" / "methods"
EXAMPLES = TREEPO_ROOT / "examples" / "research" / "methods"
...
@pytest.mark.parametrize("cls_path", [
    ...
    "treepo.methods.canonical_defaults.LmSection",
])
...
def test_unknown_top_level_key_raises() -> None:
    ...
    with pytest.raises(ValueError, match="unknown field"):
        cd.load_dataclass(path, cd.LmSection)
...
def test_overrides_skip_none() -> None:
    cfg = cd.load_dataclass(None, cd.LmSection,
                            overrides={"model": None, "temperature": 0.2})
    assert cfg.model == "nvidia/Gemma-4-31B-IT-NVFP4"  # default preserved
    assert cfg.temperature == 0.2  # override applied


def test_overrides_reject_unknown_path() -> None:
    with pytest.raises(ValueError, match="unknown"):
        cd.load_dataclass(None, cd.LmSection, overrides={"nope": 1})
