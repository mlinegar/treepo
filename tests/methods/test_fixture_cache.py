...
transfers into ``treepo.methods``.

The research LDA grid scripts sample a synthetic world ONCE and reuse it
across many cells; re-sampling would dominate runtime on large grids.
The public HLL fixture and research-only LDA fixture use the same
:func:`functools.lru_cache` pattern.
...
def test_grid_run_reuses_world_across_24_cells(tmp_path: Path) -> None:
    ...
    for oracle_name, seed, n_trees in product(
        ...
        if oracle_name == "leaf_local_mixture_target":
            trees, _cfg = make_leaf_local_mixture_trees(seed=seed)
            config["eval_data"] = list(trees)
