...
Three canonical methods (``fit`` / ``oracle`` / ``audit``).
Each one represents one orthogonal axis. Adding a new oracle, fixture,
or family does *not* require a new method — the new
thing is passed by name through config.

These tests:

1. Enumerate ``list_methods()`` and confirm all three are present.
2. Verify the oracle axis dispatches by name through one ``"oracle"`` method.
3. Verify the registry catches unknown methods AND unknown config keys
   AND unknown oracle names AND missing required keys.
...
_EXPECTED_METHODS = {"fit", "oracle", "audit"}
...
def test_methods_is_minimal_axis_factored_set() -> None:
    """The canonical method set is exactly three axis names, not a
    Cartesian product of data, oracle, and family names.
    """
...
def test_oracle_discovery_returns_registered_oracles() -> None:
    ...
    for required in ("hll_exact", "markov_changepoint_count"):
        assert required in oracles, f"oracle {required!r} not registered"
    assert "leaf_local_mixture_target" not in oracles
...
def test_oracle_domain_fixtures_advertised() -> None:
    ...
    assert "markov" in domains
    assert "lda" not in domains
...
def test_oracle_method_classical_sketch_fixture_uses_canonical_leaf_tokens(monkeypatch) -> None:
    import treepo.methods.dispatch as methods

    captured: dict = {}

    def fake_fit(spec):
        captured["leaf_token_count"] = len(spec.eval_data[0].leaves[0].tokens)
        captured["n_trees"] = len(spec.eval_data)
        return CTreePOFitResult(status="success")

    monkeypatch.setattr(methods, "fit", fake_fit)

    result = run("oracle", {"oracle_name": "hll_exact"})

    assert result.status == "success"
    assert captured["leaf_token_count"] == 24
    assert captured["n_trees"] == 6
...
def test_register_method_rejects_duplicate_without_replace() -> None:
    with pytest.raises(ValueError, match="already registered"):
        treepo.methods.register_method(
            "fit",
            lambda cfg: None,
...
def test_smoke_every_method_runs(tmp_path: Path) -> None:
    ...
    smoke_methods = set(_EXPECTED_METHODS)
