...
def test_markov_grid_oracle_matches_ground_truth_per_doc(
    ...
    methods_counts = [int(r["prediction"]) for r in rows]
    assert methods_counts == expected_counts, (
        f"seed={seed} regimes={n_regimes} tokens={max_tokens}: "
        f"counts diverged. Expected={expected_counts}, got={methods_counts}"
    )
...
def _hll_estimates_at_precision(precision: int, *, schedule: str = "balanced", tmp_path: Path) -> List[float]:
    """Read per-tree HLL estimates from the research-only sketch path."""
    ...
    trees = make_hll_token_trees(**_HLL_FIXTURE_KWARGS)
    adapter = make_hll_adapter(backend="native", precision=int(precision), hash_bits=64)
    out = []
    for tree in trees:
