...
def test_overlap_observed_rows_only() -> None:
    """Unobserved rows must not contribute to D_λ / W_λ — that was the
    cdx bug this port fixes. With one observed row at π=0.5, λ=1, the
    contributions are λ²/π = 2, λ/π = 2, ESS = 1.
    """
    rows = [
        LocalLawAuditRow(
            row_id="observed",
    ...
    assert overlap.D_lambda == pytest.approx(2.0)
    assert overlap.W_lambda == pytest.approx(2.0)
    assert overlap.effective_sample_size == pytest.approx(1.0)
