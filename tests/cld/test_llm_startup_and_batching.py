...
Both modes are controlled by ``DSPyFamilyConfig`` fields. ``treepo.cld``
...
grid run launched via ``treepo.cld.run('fit', ...)``.
...
1. **Config propagation.** A ``DSPyFamilyConfig`` with custom batch knobs
   resolves through ``treepo.cld.families.resolve_family('dspy', ...)``
...
3. **Teacher passthrough end-to-end.** The manifesto teacher backend
   ...
   ``treepo.cld.run('fit', ...)`` with a minimal teacher-passthrough
...
unit-tier proof that nothing in ``treepo.cld`` interposes between the
...
# --------------------------------------------------------------------------- #
# 1. Config propagation: knobs flow through treepo.cld unchanged.
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# 3. Teacher passthrough end-to-end through treepo.cld.run("fit", ...).
...
class _TeacherPassthroughFamily:
    ...
    protocol), so the dispatch path through treepo.cld is identical.
    """
...
def test_manifesto_teacher_passthrough_runs_through_treepo_cld(tmp_path: Path) -> None:
    ...
    reports teacher metrics. The same path through treepo.cld.run uses
...
def test_manifesto_teacher_grid_through_treepo_cld(tmp_path: Path) -> None:
    """A grid run over a manifesto-style axis (dimension) using the
    teacher path. Verifies the same shape works at scale through
    ``treepo.cld.run`` — same loop a paper grid script would write.
    """
    dim_scores_by_dim = {
