...
path works end-to-end through ``treepo.cld.run`` against a real GPU server.
...
Enable with:
    ...
    TT_RUN_LIVE_TESTS=1 \\
      python -m pytest \\
      treepo.cld/tests/integration/test_manifesto_dspy_live.py -v
"""
...
def test_manifesto_dspy_inference_against_live_gemma_lands_near_paper_pearson(
    ...
    - Use the pretuned ``DimensionScorer`` at
      ...
      as ``f_init``.
    - Run ``treepo.cld.run("fit", {"family": "dspy", ..., "axis":
    ...
    ``treepo.cld`` path reproduces it end to end.
    """
