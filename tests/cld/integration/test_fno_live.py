"""Live FNO training cell through ``treepo.cld.run`` on GPU.
...
- ``treepo.cld.families.resolve_family("fno", ...)`` builds the same
  ``FNOFamily`` the paper code constructs.
- ``treepo.cld.run("fit", {family="fno", ...})`` completes one ``f``
...
def test_live_fno_family_trains_one_iteration_through_treepo_cld(tmp_path: Path) -> None:
    """Smallest FNO training cell that exercises CUDA + the
    ``FamilyRuntime`` protocol end-to-end through the treepo.cld
    dispatcher.
    """
