...
Pattern: two sections in the TOML, each into its own dataclass:
  ...
  [lm]       → LmSection (treepo.cld scenario wrapper)
"""
...
def main() -> int:
    ...
    ap.add_argument("--config", type=Path,
                    default=REPO_ROOT / "treepo.cld/configs/manifesto_fg_compile.toml")
    ap.add_argument("--budget", default=None)
    ap.add_argument("--vllm-urls", default=None, help="comma-separated endpoint list")
    args = ap.parse_args()

    import treepo.cld
    from treepo.cld.canonical_defaults import build_lm_config_dict, load_dataclass, LmSection
