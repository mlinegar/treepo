...
Run all three reference views with
...
- **Collapsed row**: node id, a unit-type chip, a sampled (●) / unsampled (○)
  ...
  `prediction`, …; configurable via `label_keys=`), orange/red chips for
  local-law proxy/oracle losses, and an inline gray snippet of the node's
  text.
...
**Markov** (`write_markov_law_view`) — learned family against exact
supervision:
...
write_tree_visualization_html(markov_tree_records(trees), path, law_rows=law_rows)
```
...
model's per-node local-law losses next to the ground truth on the
synthesized merge tree.
