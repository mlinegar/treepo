...
```
_fno_loss          joint named-vector endpoint loss policy                     (bottom)
   ▲
_fno_config        constants, Config dataclasses, coercion, clamp/safe_float   (machinery)
   ▲
_fno_neuralop      torch/neuralop discovery, constructor kwargs, validation    (machinery)
   ▲
_fno_models        _UnifiedGTreeModel + one shared leaf/merge g                 (model logic)
...
The optimizer-backed DSPy family is split at the same orchestration/data seam:

- `dspy.py` is the stable public family surface. It owns configuration,
  prediction parsing, and the lean runtime methods that delegate record
  construction and optimization.
- `_dspy_records.py` owns DSPy training data: node/`TaskState` extraction,
  topology-aware leaf/unary-recompression/binary-merge records, role weights,
  and group-disjoint train/validation splitting.
- `_dspy_optimizer.py` owns compiler integration, warm starts, program
  persistence/reload, and recursive reduction through the one shared `g`.

This split keeps provider/compiler machinery and row construction out of the
public parser/config surface without introducing a second learning API.
`_dspy_records.py` also owns the strict binary-topology validation used by
preference export, and canonical propensity resolution lives below both DSPy
and preference ingestion in `treepo.sampling`. Training artifacts record
whether an `f` compile used raw identity input, a reference state, a preference
prompt, or a state generated by the current `g`. Persistence validates the
ordered target/oracle catalog plus signature instructions before warm start;
state-only JSON is the safe default, while whole-program pickle loading is an
explicit trusted-artifact opt-in.

### DSPy source ledger

The recovered standalone source anchor is
`af1dfef6c4d3e670cd47fcf8d231a21a4615a652`; its mirrored pre-PolMeth
ThinkingTrees anchor is `a7dbb96123cc56a372e4527689104351e832e9d2`. The
current task-neutral adaptation preserves optimizer-backed alternating
`f`/current-`g` and current-`f`/shared-`g` semantics; it does not restore the
deleted `_research` dependencies wholesale. The latest qsentence refinements
remain in the ThinkingTrees source.

In particular, shared-`g` compiler learning, scheduled sampling, persistence,
and the alternating `f -> g -> f` path are recovered behavior, not a second
implementation invented for the vector API. The adaptation removes
scalar/positional special cases: K=1 is a one-coordinate named vector and uses
the same record, compiler, persistence, and inference path as K>1. Task-owned
signature prose supplies RILE meaning without forking the learner. Any future
change should be compared against the two anchors above before adding a
...
`lda.py` (the LDA fixture baseline helpers), and `llm.py` (the provider-neutral
inference wrapper family). The decomposed DSPy family is described above.
