...
The paired Semantic-Forest grid uses `max_iterations=3`, so learned cells
execute `f -> g -> f`; identity/fixed-`g` cells elide the `g` slot. Its learned
DSPy backend sets `f_record_source="generated_when_available"`. The initial
