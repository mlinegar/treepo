"""Paper-canonical defaults for every `treepo.cld` family.
...
1. **Truth lives upstream.** When the main repo already has a dataclass
   ...
   ``MarkovChangepointConfig``, ``LDATreeRecoveryConfig``), the TOML
...
Usage::
    ...
    cfg = load_dataclass("treepo.cld/configs/fno_smoke.toml", FNOFamilyConfig)
    treepo.cld.run("fit", {"family": "fno", "backend_config": {"fno_config": cfg, ...}, ...})

See ``treepo.cld/docs/training_defaults.md`` for the full table of
...
# Make the parent repo importable so upstream dataclasses resolve.
...
from treepo._research.training.optimization.gepa import (  # noqa: E402
...
class HllSketchConfig:
    """HLL adapter knobs. Mirrors ``methods._factory_hll`` literals + sketch schedule.

    No upstream dataclass for these — the defaults live as ``.get`` fallbacks
    in the factory. Mirrored here as the single source so the TOML and
    example can share them. ``precision`` defaults to 14 (paper grid's
    min-MAE point), not 12 (the factory fallback).
    """

    backend: str = "native"
    precision: int = 14
    hash_bits: int = 64
    schedule: str = "balanced"  # "balanced" | "left_to_right" | "right_to_left"
    # Fixture knobs (used when the example doesn't pass explicit eval_data).
    n_trees: int = 6
...
@dataclass
class LdaOracleConfig:
    """LDA leaf-local-mixture oracle dispatch knobs."""
...
__all__ = [
    ...
    "LmSection",
    # Family-specific small configs (no upstream dataclass)
    "HllSketchConfig", "LdaOracleConfig",
