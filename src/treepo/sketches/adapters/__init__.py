...
from treepo.sketches.protocol import CardinalitySketch
...
def make_hll_adapter(
    *,
    ...
    - `backend="datasketches"` wraps `datasketches.hll_sketch`. Requires
      `pip install 'treepo[sketches]'`; raises a clear `ImportError` otherwise.
    """
    if backend == "native":
        from treepo.sketches.adapters.hll_native import HLLNativeAdapter
        return HLLNativeAdapter(precision=int(precision), hash_bits=int(hash_bits))
    if backend == "datasketches":
        from treepo.sketches.adapters.hll_datasketches import HLLDatasketchesAdapter
...
def make_cpc_adapter(*, lg_k: int = 10) -> CardinalitySketch:
    from treepo.sketches.adapters.datasketches_cardinality import CPCDatasketchesAdapter

    return CPCDatasketchesAdapter(lg_k=int(lg_k))


def make_theta_adapter(*, lg_k: int = 12) -> CardinalitySketch:
    from treepo.sketches.adapters.datasketches_cardinality import ThetaDatasketchesAdapter

    return ThetaDatasketchesAdapter(lg_k=int(lg_k))


def make_count_min_adapter(*, num_hashes: int = 5, num_buckets: int = 256):
    from treepo.sketches.adapters.datasketches_frequency import CountMinDatasketchesAdapter
...
def make_frequent_strings_adapter(*, lg_max_map_size: int = 8):
    from treepo.sketches.adapters.datasketches_frequency import FrequentStringsDatasketchesAdapter

    return FrequentStringsDatasketchesAdapter(lg_max_map_size=int(lg_max_map_size))


def make_kll_floats_adapter(*, k: int = 200):
    from treepo.sketches.adapters.datasketches_quantiles import KLLFloatsDatasketchesAdapter

    return KLLFloatsDatasketchesAdapter(k=int(k))


def make_quantiles_floats_adapter(*, k: int = 128):
    from treepo.sketches.adapters.datasketches_quantiles import QuantilesFloatsDatasketchesAdapter

    return QuantilesFloatsDatasketchesAdapter(k=int(k))


def make_req_floats_adapter(*, k: int = 12, high_rank_accuracy: bool = True):
    from treepo.sketches.adapters.datasketches_quantiles import REQFloatsDatasketchesAdapter
...
def make_tdigest_double_adapter(*, k: int = 200):
    from treepo.sketches.adapters.datasketches_quantiles import TDigestDoubleDatasketchesAdapter

    return TDigestDoubleDatasketchesAdapter(k=int(k))


def make_tuple_accumulator_adapter(*, lg_k: int = 12):
    from treepo.sketches.adapters.datasketches_tuple_sampling import (
