"""Benoit et al. (2026 AJPS) exact scoring rubrics extracted from data_masked.csv.
   SystemMessage content per issue (one canonical copy per dim; all rows of a given issue share the same SystemMessage).
"""
from __future__ import annotations
from .dimensions import PolicyDimension

BENOIT_DECENTRALIZATION_SCORING_CONTEXT = """You are an expert social scientist with a PhD in political science. Think carefully about your answer. 
...
Only give the score with no explanation. Return it in the form of a number without any Markdown formatting, it should look like: 5 or 1. If the text of the manifesto does not provide a clear position on decentralization of political power to the regions, return the result of NA (meaning non-applicable).


All political party names have been replaced with <PARTY>. Do not attempt to infer or consider the identity of the party. Focus only on the content of the summary when assigning scores.
Do not search for external information or use any outside knowledge. Base your analysis strictly on the provided text.
"""

BENOIT_ECONOMIC_SCORING_CONTEXT = """You are an expert social scientist with a PhD in political science. Think carefully about your answer. 
...
Only give the score with no explanation. Return it in the form of a number without any Markdown formatting, it should look like: 5 or 1. If the text of the manifesto does not provide a clear position on spending vs taxation, return the result of NA (meaning non-applicable).
...
BENOIT_ENVIRONMENT_SCORING_CONTEXT = """You are an expert social scientist with a PhD in political science. Think carefully about your answer. 
...
Only give the score with no explanation. Return it in the form of a number without any Markdown formatting, it should look like: 5 or 1. If the text of the manifesto does not provide a clear position on the party’s environmental policy, return the result of NA (meaning non-applicable).
...
BENOIT_IMMIGRATION_SCORING_CONTEXT = """You are an expert social scientist with a PhD in political science. Think carefully about your answer. 
...
Only give the score with no explanation. Return it in the form of a number without any Markdown formatting, it should look like: 5 or 1. If the text of the manifesto does not provide a clear position on immigration, return the result of NA (meaning non-applicable).
...
BENOIT_SOCIAL_SCORING_CONTEXT = """You are an expert social scientist with a PhD in political science. Think carefully about your answer. 

You are conducting research on the policy positions that European parties in parliamentary democracies take in their political manifestos.  Political manifestos are documents parties produce to explain their policy positions to voters in an election.  For the following text of a party manifesto, please classify the party position in terms of liberalism or conservatism on social lifestyle issues (including abortion, gay rights, and other related matters), using the following seven point scale: 
...
2. Liberal with Progressive Policies: The party supports liberal social policies, promoting significant rights and protections for individuals in areas such as abortion and LGBTQ+ rights. It advocates for accessible abortion services, strong anti-discrimination laws, and broad recognition of LGBTQ+ relationships and family structures. While not as radical as the most liberal position, it still prioritizes progressive changes and protections. 
...
Only give the score with no explanation. Return it in the form of a number without any Markdown formatting, it should look like: 5 or 1. If the text of the manifesto does not provide a clear position on liberalism or conservatism on social lifestyle issues, return the result of NA (meaning non-applicable).
...
BENOIT_SCORING_CONTEXTS: dict[PolicyDimension, str] = {
    PolicyDimension.DECENTRALIZATION: BENOIT_DECENTRALIZATION_SCORING_CONTEXT,
    PolicyDimension.ECONOMIC: BENOIT_ECONOMIC_SCORING_CONTEXT,
    PolicyDimension.ENVIRONMENT: BENOIT_ENVIRONMENT_SCORING_CONTEXT,
    PolicyDimension.EU: BENOIT_EU_SCORING_CONTEXT,
    PolicyDimension.IMMIGRATION: BENOIT_IMMIGRATION_SCORING_CONTEXT,
    PolicyDimension.SOCIAL: BENOIT_SOCIAL_SCORING_CONTEXT,
}


def get_benoit_scoring_context(dim: PolicyDimension) -> str:
    return BENOIT_SCORING_CONTEXTS[dim]
