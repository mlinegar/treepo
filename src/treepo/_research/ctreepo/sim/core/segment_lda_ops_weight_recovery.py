...
The oracle target is a mergeable linear functional of the *latent* topic sequence:
    ...
    f⋆(span) = <θ, topic_counts(span)> + quadratic_weight * <W, topic_bigrams(span)>
...
- the quadratic weight is a scalar multiplier for the bigram term (we include it in the grid).
...
from treepo._research.ctreepo.sim.core.training_selection import TrainingSelectionMetadata
...
def _train_topic_operator_network(
    *,
    ...
    """
    Fit a lightweight neural operator that maps estimated topic vectors to refined vectors.
    ...
    try:
        ...
        return pred, {
            "backend": "kernel_ridge_fallback",
            "train_loss": float("nan"),
            **TrainingSelectionMetadata(
                mode="analytic_kernel_ridge",
                split="config",
                metric_name="train_loss",
                metric_value=float("nan"),
                best_epoch=0,
            ).to_dict(),
            "device_requested": str(validate_device_mode(str(device))),
            "device_used": "cpu",
            "device_mode_resolved": "cpu",
        }
    ...
    x_t = torch.tensor(x_train, dtype=torch.float32, device=torch_device)
    y_t = torch.tensor(y_train, dtype=torch.float32, device=torch_device)
    x_all_t = torch.tensor(_normalize_topic_rows(xa), dtype=torch.float32, device=torch_device)
    ...
    if mode_l == "ctreepo":
        ...
        model: nn.Module = _CTreePONet(v, h).to(torch_device)
    else:
        ...
        model = _MergeableNet(v, h).to(torch_device)
    ...
    backend = "torch_cuda" if getattr(torch_device, "type", None) == "cuda" else "torch_cpu"
    return out, {
        "backend": backend,
        "train_loss": float(last_loss),
        **TrainingSelectionMetadata(
            mode="final_step_no_validation",
            split="config",
            metric_name="train_loss",
            metric_value=float(last_loss),
            best_epoch=max(0, int(n_steps) - 1),
        ).to_dict(),
        **runtime_meta,
    }
...
def _neural_refine_topics(
    ...
    *,
    ...
    """
    Refine topic-word estimates using lightweight neural operators.
    ...
    if not np.all(np.isfinite(pred_ms)):
        ...
        ms_meta = {
            "backend": "kernel_ridge_numeric_fallback",
            "train_loss": float("nan"),
            **TrainingSelectionMetadata(
                mode="analytic_kernel_ridge",
                split="config",
    ...
    meta: Dict[str, Any] = {
        ...
        "topic_phi_neural_mergeable_selection_mode": str(
            ms_meta.get("selection_mode", "unknown")
        ),
        "topic_phi_neural_mergeable_selection_split": str(
            ms_meta.get("selection_split", "config")
        ),
        "topic_phi_neural_mergeable_selection_metric_name": str(
            ms_meta.get("selection_metric_name", "train_loss")
        ),
        "topic_phi_neural_mergeable_selection_metric_value": float(
            ms_meta.get("selection_metric_value", float("nan"))
        ),
        "topic_phi_neural_mergeable_best_epoch": int(ms_meta.get("best_epoch", 0)),
    }
...
def estimate_topic_distributions(
    ...
    *,
    ...
    requested_device = str(validate_device_mode(str(device)))
    cpu_runtime_meta: Dict[str, object] = {
...
class SegmentLDAOpsWeightRecoveryConfig:
    ...
    # Legacy internal name for the quadratic topic-bigram weight.
...
@dataclass(frozen=True)
class SegmentLDAOpsWeightRecoveryWorld:
    ...
    W_base: np.ndarray
    docs_train: Tuple[SegmentLDADoc, ...]
    docs_test: Tuple[SegmentLDADoc, ...]
    docs_phi_extra: Tuple[SegmentLDADoc, ...]


def _resolved_boundary_profile_seed(config: SegmentLDAOpsWeightRecoveryConfig) -> int:
    ...
    return int(boundary_profile_seed)


def _world_signature(config: SegmentLDAOpsWeightRecoveryConfig) -> Dict[str, object]:
    process = str(config.topic_process).strip().lower()
    sig = {
        "seed": int(config.seed),
        "boundary_profile_seed": int(_resolved_boundary_profile_seed(config)),
    ...
    if process != "bag_of_words":
        sig["leaf_tokens"] = int(config.leaf_tokens)
    return sig


def segment_lda_ops_weight_recovery_world_cache_signature(
    config: SegmentLDAOpsWeightRecoveryConfig,
    *,
    train_docs_capacity: int,
    test_docs_capacity: int,
    phi_extra_docs_capacity: int,
) -> Dict[str, object]:
    sig = dict(_world_signature(config))
    sig.update(
        {
            "train_docs_capacity": int(train_docs_capacity),
            "test_docs_capacity": int(test_docs_capacity),
            "phi_extra_docs_capacity": int(phi_extra_docs_capacity),
        }
    )
    return sig


def _topic_runs(topics: Sequence[int]) -> List[Tuple[int, int, int]]:
    z = np.asarray(topics, dtype=np.int64)
    if z.size == 0:
        return []
    ...
    start = 0
    prev = int(z[0])
    for idx in range(1, int(z.size)):
        cur = int(z[idx])
        if cur != prev:
            runs.append((int(start), int(idx), int(prev)))
            start = int(idx)
            prev = cur
    runs.append((int(start), int(z.size), int(prev)))
    return runs


def _summarize_generated_docs(
    ...
    *,
    ...
    for doc in docs:
        leaves = build_leaf_spans(len(doc.tokens), leaf_tokens=leaf)
        n_leaves = len(leaves)
        topics_arr = np.asarray(doc.topics, dtype=np.int64)

        if process == "segments":
            runs = _topic_runs(doc.topics)
            segment_stats.append(int(len(runs)))
            for start, end, _topic in runs:
                covered = 0
                for leaf_start, leaf_end in leaves:
                    if int(leaf_end) <= int(start):
                        continue
                    if int(leaf_start) >= int(end):
                        break
                    covered += 1
                if covered > 0:
                    segment_len_leaves_all.append(int(covered))
        else:
            segment_stats.append(0)
        ...
        purities: List[float] = []
        for start, end in leaves:
            seg = topics_arr[int(start) : int(end)]
            if seg.size == 0:
                continue
            uniq, counts = np.unique(seg, return_counts=True)
            best = int(np.argmax(counts))
            leaf_topics.append(int(uniq[best]))
            purities.append(float(counts[best]) / float(seg.size))
    ...
    stats = {
        "mean_segments": float(np.mean(np.asarray(segment_stats, dtype=np.float64))) if segment_stats else 0.0,
        "mean_changepoints": float(np.mean(np.asarray(changepoints, dtype=np.float64))) if changepoints else 0.0,
        "mean_changepoint_rate": float(np.mean(np.asarray(changepoint_rates, dtype=np.float64)))
        if changepoint_rates
        else 0.0,
        "mean_leaf_topic_purity": float(np.mean(np.asarray(leaf_purities, dtype=np.float64))) if leaf_purities else 0.0,
    }
    if segment_len_leaves_all:
        seg_lens = np.asarray(segment_len_leaves_all, dtype=np.float64)
        stats.update(
...
def _ridge_run_to_metrics_dict(run: RidgeRunMetrics) -> Dict[str, object]:
    return {
        **asdict(run.sketch),
        **asdict(run.weights),
        **asdict(run.topic_inference),
        **asdict(run.budget),
        **asdict(run.design),
    }
...
def sample_segment_lda_ops_weight_recovery_world(
    config: SegmentLDAOpsWeightRecoveryConfig,
    *,
    train_docs_capacity: Optional[int] = None,
    test_docs_capacity: Optional[int] = None,
    phi_extra_docs_capacity: int = 0,
) -> SegmentLDAOpsWeightRecoveryWorld:
    ...
    phi_extra_cap = int(max(0, phi_extra_docs_capacity))
    ...
    boundary_profile_seed = _resolved_boundary_profile_seed(config)
    ...
    docs_phi_extra: Tuple[SegmentLDADoc, ...] = tuple()
    if phi_extra_cap > 0:
        docs_phi_extra, _phi_extra_stats = generate_segment_lda_docs(
            phi_extra_cap,
            topics=topics_phi,
            min_tokens=int(config.min_tokens),
            max_tokens=int(config.max_tokens),
    ...
    return SegmentLDAOpsWeightRecoveryWorld(
        signature=_world_signature(config),
        boundary_profile_seed=int(boundary_profile_seed),
        topic_meta=dict(topic_meta),
        topics_phi=tuple(topics_phi),
        relevant_topics=tuple(relevant_topics),
        ...
        docs_phi_extra=tuple(docs_phi_extra),
    )


def run_segment_lda_ops_weight_recovery_experiment_from_world(
    config: SegmentLDAOpsWeightRecoveryConfig,
    world: SegmentLDAOpsWeightRecoveryWorld,
    ...
    relevant_topics = tuple(int(x) for x in world.relevant_topics)
    ...
    phi_from_train = int(min(len(docs_train), phi_docs_effective))
    phi_extra = int(max(0, phi_docs_effective - len(docs_train)))
    if phi_extra > len(world.docs_phi_extra):
        raise ValueError("config.topic_phi_docs requires more extra docs than the fixed world provides")
    docs_phi = list(docs_train[:phi_from_train]) + list(world.docs_phi_extra[:phi_extra])
    ...
    train_stats = _summarize_generated_docs(
        docs_train,
        leaf_tokens=int(config.leaf_tokens),
        topic_process=str(config.topic_process),
    )
    test_stats = _summarize_generated_docs(
        docs_test,
        leaf_tokens=int(config.leaf_tokens),
        topic_process=str(config.topic_process),
    )
    phi_extra_stats = _summarize_generated_docs(
        world.docs_phi_extra[:phi_extra],
        leaf_tokens=int(config.leaf_tokens),
        topic_process=str(config.topic_process),
    ) if phi_extra > 0 else {}
    ...
    metrics = {
        ...
        "ridge": _ridge_run_to_metrics_dict(ridge),
    }
    if ridge_true_topics is not None:
        metrics["ridge_true_topics"] = _ridge_run_to_metrics_dict(ridge_true_topics)
    if ridge_infer_true_phi is not None:
        metrics["ridge_infer_true_phi"] = _ridge_run_to_metrics_dict(ridge_infer_true_phi)
    ...
    return SegmentLDAOpsWeightRecoverySummary(
        ...
        topic_meta={
            ...
            **topic_est_meta,
            ...
            "world_fixed_documents": True,
            "world_train_docs_capacity": float(len(world.docs_train)),
            "world_test_docs_capacity": float(len(world.docs_test)),
            "world_phi_extra_docs_capacity": float(len(world.docs_phi_extra)),
            "world_boundary_profile_seed": float(world.boundary_profile_seed),
...
def run_segment_lda_ops_weight_recovery_experiment(
    ...
    if str(getattr(config, "device", "auto")).strip().lower() not in VALID_DEVICE_MODES:
        raise ValueError(f"device must be one of {VALID_DEVICE_MODES}")
    if int(getattr(config, "torch_threads", 0)) < 0:
    ...
    topics_phi_est, topic_est_meta, perm_est_to_true = estimate_topic_distributions(
        ...
        cuda_device=getattr(config, "cuda_device", None),
        torch_threads=int(getattr(config, "torch_threads", 0)),
    )
